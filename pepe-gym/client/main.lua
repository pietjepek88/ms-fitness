        ...
        local coords = GetEntityCoords(PlayerPedId())
            for i, v in pairs(Config.Locations) do
                ...
                if dist <= 1.5 and not exercising then
                    sleep = 5
                    Framework.Functions.DrawText3D(pos["x"], pos["y"], pos["z"] + 0.98, "[E] " .. pos["exercise"])
                    if IsControlJustPressed(0, Keys["E"]) then
                        startExercise(Config.Exercises[pos["exercise"]], pos)
                    end
                    else if dist <= 3.0 and not exercising then
                        sleep = 8
                        Framework.Functions.DrawText3D(pos["x"], pos["y"], pos["z"] + 0.98, pos["exercise"])
                    end
                end
            end
        Citizen.Wait(sleep)
    ...
    Citizen.CreateThread(function()
        while canExercise do
            ...
            if procent <= 99 then
                ...
                ExitTraining(animInfo["exitDict"], animInfo["exitAnim"], animInfo["exitTime"])
                local randomStamina = math.random()
                local randomStrenght = math.random()
                TriggerServerEvent('pepe-skills:addStamina', GetPlayerServerId(PlayerId()), (randomStamina + 0))
                TriggerServerEvent('pepe-skills:addStrength', GetPlayerServerId(PlayerId()), (randomStrenght + 0))
                Framework.Functions.Notify("Je kreeg " .. string.format('% 0.2f', randomStamina) .. " Stamina erbij", "success")
                Framework.Functions.Notify("Je kreeg " .. string.format('% 0.2f', randomStrenght) .. " Strenght erbij!", "success")
